...
//!
//! ```
//! const AtomicU128Counter = Rc(u128, Atomic);
...
//! ```
//! (from https://github.com/yrashk/zig-rcsp)
...
pub const Atomic = if (b.single_threaded) NonAtomic else struct {
    const T = usize;
    pub const MAX = s.math.maxInt(T);
    pub const MIN = s.math.minInt(T);
...
test "saturating atomic increment from max usize" {
    var val: usize = s.math.maxInt(usize);
    testing.expectEqual(@as(usize, @intCast(s.math.maxInt(usize))), Atomic.increment(&val));
    testing.expectEqual(@as(usize, @intCast(s.math.maxInt(usize))), Atomic.load(&val));
}
...
fn increment100(val: *usize) void {
    ...
    while (i < 100) {
        ...
        s.time.sleep(10 * s.time.millisecond);
...
test "concurrent atomic increments & decrements" {
    var val: usize = 0;
    const t1 = try s.Thread.spawn(&val, increment100);
    const t2 = try s.Thread.spawn(&val, increment100);
    t1.wait();
    t2.wait();
    testing.expectEqual(@as(usize, @intCast(200)), Atomic.load(&val));
    const t3 = try s.Thread.spawn(&val, decrement100);
    const t4 = try s.Thread.spawn(&val, decrement100);
    t3.wait();
    t4.wait();
    testing.expectEqual(@as(usize, @intCast(0)), Atomic.load(&val));
}
...
test "saturating concurrent atomic increments" {
    var val: usize = s.math.maxInt(usize) - 100;
    const t1 = try s.Thread.spawn(&val, increment100);
    const t2 = try s.Thread.spawn(&val, increment100);
    t1.wait();
    t2.wait();
...
test "saturating concurrent atomic decrements" {
    var val: usize = s.math.minInt(usize) + 100;
    const t1 = try s.Thread.spawn(&val, decrement100);
    const t2 = try s.Thread.spawn(&val, decrement100);
    t1.wait();
    t2.wait();
    testing.expectEqual(@as(usize, @intCast(s.math.minInt(usize))), Atomic.load(&val));
}
...
test "non-atomic increment from max usize" {
    var val: usize = s.math.maxInt(usize);
    testing.expectEqual(NonAtomic.increment(&val), s.math.maxInt(usize));
    testing.expectEqual(NonAtomic.load(&val), s.math.maxInt(usize));
}
...
pub fn Rc(comptime T: type, Ops: type) type {
    ...
    return struct {
        ...
        pub const Weak = struct {
            ...
            // creation of Rc if the constant below
        ...
        pub fn init(val: T, allocator: *s.mem.Allocator) !Self {
        ...
        /// var counter = Rc(T, Atomic).init(T{ .value = 10 });
...
test "initializing a shared pointer with a value" {
    var v = try Rc(u128, NonAtomic).init(10, s.heap.page_allocator);
    defer _ = v.deinit();
...
test "safely mutating shared pointer's value" {
    const MU128 = struct {
        value: u128,
        mutex: s.Mutex = s.Mutex.init(),
    };
    var mutex = MU128{ .value = 10 };
    var v = try Rc(MU128, NonAtomic).init(mutex, s.heap.page_allocator);
    defer _ = v.deinit();
    const ptr = v.unsafePtr();
    {
...
fn deinit50(val: *[50]Rc(u128, Atomic)) void {
    var i: usize = 0;
    var r: usize = 0;
...
test "deinitializing atomic reference counter" {
    const T = Rc(u128, Atomic);
    var c: usize = 0;
    // try this many times
    while (c < 100_000) : (c += 1) {
        @atomicStore(usize, &deinit50c, 0, .SeqCst);
        var v = try T.init(10, s.heap.page_allocator);
        var i: usize = 0;
        var clones = try s.ArrayList(T).initCapacity(s.heap.page_allocator, 150);
        try clones.append(v);
        while (i < 149) : (i += 1) {
        ...
        const t1 = try s.Thread.spawn(clones.items[0..50], deinit50);
        const t2 = try s.Thread.spawn(clones.items[50..100], deinit50);
        const t3 = try s.Thread.spawn(clones.items[100..150], deinit50);
...
fn deinit50_alloc(val: *[50]Rc(u128, Atomic)) void {
    var i: usize = 0;
    // wait until a signal to go is given
    while (!@atomicLoad(bool, &deinit50s, .SeqCst)) {}
    while (i < 50) : (i += 1) {
...
fn deinit50w_alloc(val: *[50]Rc(u128, Atomic).Weak) void {
    var i: usize = 0;
    // wait until a signal to go is given
    while (!@atomicLoad(bool, &deinit50s, .SeqCst)) {}
    while (i < 50) : (i += 1) {
...
test "deallocating atomic reference counter" {
    ...
    while (c < 100_000) : (c += 1) {
        ...
        var v = try T.init(10, s.heap.page_allocator);
        var i: usize = 0;
        var clones = try s.ArrayList(T).initCapacity(s.heap.page_allocator, 100);
        try clones.append(v);
        while (i < 99) : (i += 1) {
            try clones.append(v.strongClone());
        }

        var wclones = try s.ArrayList(T.Weak).initCapacity(s.heap.page_allocator, 50);
        i = 0;
        while (i < 50) : (i += 1) {
        ...
        @atomicStore(bool, &deinit50s, false, .SeqCst);
        const t1 = try s.Thread.spawn(clones.items[0..50], deinit50_alloc);
        const t2 = try s.Thread.spawn(clones.items[50..100], deinit50_alloc);
        const t3 = try s.Thread.spawn(wclones.items[0..50], deinit50w_alloc);
        @atomicStore(bool, &deinit50s, true, .SeqCst);
        t1.wait();
        t2.wait();
        t3.wait();
